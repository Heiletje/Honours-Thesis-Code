...
# Function: Provide the necessary functions for calibrating a multivariate Renewal Hawkes process using the exact MLE
            Assumption: immigrant labels are known 
- Structure:
	(1) Intensity functions with supporting functions
            (a) renewal intensity (Weibull) 
            (b) immigrant event intensity at time t 
            (c) intensity function for exponential (Exp) memory kernel
            (d) intensity function for power law (PL) memory kernel 

	(2) Integrated intensity function 
            (a) for exponential (Exp) memory kernel
            (b) for power law (PL) memory kernel 

	(3) Log-likelihood 
            (a) for exponential (Exp) memory kernel
            (b) for power law (PL) memory kernel 
            
	(4) Generalised residuals (GR)
            (a) for exponential (Exp) memory kernel
            (b) for power law (PL) memory kernel 
=#
#---------------------------------------------------------------------------------------------------
...
# (b) Immigrant intensity at time t
...
# (c) Extract the Intensity fuction given the observations for exponetial kernel
...
# (d) Extract the Intensity fuction given the observations for power law kernel
...
# (a) Function to compute the integrated intensity from [0,T] ∫_0^T λ^m(t) dt given the observations for exponential kernel
...
# (b) Function to compute the integrated intensity from [0,T] ∫_0^T λ^m(t) dt given the observations for power law kernel
...
# (a) Computes the partial log-likelihoods and sums them up to obtain the full log-likelihood for exponential kernel
...
# (b) Computes the partial log-likelihoods and sums them up to obtain the full log-likelihood for power law kernel
...
# (a) Computes the Generalised Residuals for the exponential kernel
...
# (b) Computes the Generalised Residuals for the power law kernel
